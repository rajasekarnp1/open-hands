...
def patch_agent_and_checkpoint_manager(
    ...
    with patch('src.api.server.code_agent_instance') as mock_agent_global_ref:
        # Configure the 'generate_code' method of the globally referenced instance
        if mock_agent_global_ref: # It might not be initialized if lifespan doesn't run in test setup
             mock_agent_global_ref.generate_code = mock_code_agent_generate_code

        with patch('src.api.server.checkpoint_manager_instance') as mock_cm_global_ref:
...
def test_resume_agent_success(
    ...
    # Check that generate_code was called for resumption
    mock_code_agent_generate_code.assert_called_once()
    resumed_call_request_arg = mock_code_agent_generate_code.call_args[0][0]
    assert resumed_call_request_arg.thread_id == thread_id
    assert resumed_call_request_arg.instruction == "Continue based on human feedback."
    assert resumed_call_request_arg.project_directory == "/test" # From original_request_info
    assert resumed_call_request_arg.language == "python"
